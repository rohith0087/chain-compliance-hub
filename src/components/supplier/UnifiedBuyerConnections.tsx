...
const UnifiedBuyerConnections = ({ onConnectionRequest }: UnifiedBuyerConnectionsProps) => {
  ...
  const getConnectionDisplayStatus = (connection: any) => {
    ...
    if (connection.status === 'approved') {
      ...
      const statusMap: any = { requested: { status: 'onboarding_requested', label: 'Onboarding Requested', color: 'bg-purple-100 text-purple-800', icon: <Clock className="w-4 h-4 text-purple-500" />, action: 'awaiting_buyer', description: 'Waiting for buyer to approve' }, pending: { status: 'onboarding_pending', label: 'Onboarding Pending', color: 'bg-amber-100 text-amber-800', icon: <AlertCircle className="w-4 h-4 text-amber-500" />, action: 'start_onboarding', description: 'Ready to start onboarding' }, onboarding_initiated: { status: 'onboarding_in_progress', label: 'Onboarding In Progress', color: 'bg-blue-100 text-blue-800', icon: <Clock className="w-4 h-4 text-blue-500" />, action: 'continue_onboarding', description: 'Complete remaining steps' }, under_review: { status: 'under_review', label: 'Under Review', color: 'bg-yellow-100 text-yellow-800', icon: <Clock className="w-4 h-4 text-yellow-500" />, action: 'awaiting_buyer', description: 'Buyer is reviewing' }, approved: { status: 'fully_connected', label: 'Fully Connected', color: 'bg-green-100 text-green-800', icon: <CheckCircle className="w-4 h-4 text-green-500" />, action: 'none', description: 'Onboarding complete' } };
