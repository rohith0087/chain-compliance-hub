...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Get the document request details with buyer info
    const { data: request, error: requestError } = await supabase
      .from("document_requests")
      .select(`
        id, title, description, document_type, category, priority, due_date,
        branch_id, supplier_branch_id, buyer_id,
        buyers ( company_name )
      `)
      .eq("id", requestId)
      .single();
    ...
    // Get branch users if request targets a specific supplier branch
          ...
          <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
            ...
            <div style="background: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 8px 0; color: #6b7280; width: 120px;">📄 Request:</td><td style="padding: 8px 0; font-weight: 600;">${request.title}</td></tr>
                <tr><td style="padding: 8px 0; color: #6b7280;">🏢 From:</td><td style="padding: 8px 0;">${buyerName}</td></tr>
                <tr><td style="padding: 8px 0; color: #6b7280;">📁 Category:</td><td style="padding: 8px 0;">${request.category || 'N/A'}</td></tr>
                <tr><td style="padding: 8px 0; color: #6b7280;">⚡ Priority:</td><td style="padding: 8px 0;"><span style="background: ${priorityColor}; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px; text-transform: uppercase;">${request.priority || 'Medium'}</span></td></tr>
                ${request.due_date ? `<tr><td style="padding: 8px 0; color: #6b7280;">📅 Due Date:</td><td style="padding: 8px 0;">${new Date(request.due_date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</td></tr>` : ''}
                ${(request as any).target_branch_name ? `<tr><td style="padding: 8px 0; color: #6b7280;">🏢 Your Branch:</td><td style="padding: 8px 0;">${(request as any).target_branch_name}</td></tr>` : ''}
              </table>
            </div>
            ${request.description ? `<div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px 16px; margin-bottom: 24px; border-radius: 0 8px 8px 0;"><p style="margin: 0; font-size: 14px; color: #92400e;"><strong>Description:</strong> ${request.description}</p></div>` : ''}
            <div style="text-align: center; margin: 30px 0;">
              <a href="https://compliance.tracer2c.com/dashboard" style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-block;">View & Respond</a>
            </div>
