-- Slice 7: scoring engine v2 — feedback-aware.
-- Risk events are buyer-AGNOSTIC shared knowledge, so a buyer's "not relevant" /
-- "false entity match" label must only affect THAT buyer's score (never global).
-- v2 excludes events the calling buyer has labelled as such. Still deterministic,
-- no LLM. Verified: Deb El 14 -> 8 after labelling 1 of 2 events not_relevant.
...
BEGIN
  ...
  WITH active_events AS (
    ...
    WHERE e.supplier_id = p_supplier_id
      ...
      -- v2: honour this buyer's own feedback (buyer-scoped, never global)
      AND NOT EXISTS (
        SELECT 1 FROM supplier_risk_feedback f
        WHERE f.risk_event_id = e.id
          AND f.buyer_id = p_buyer_id
          AND f.feedback_type IN ('not_relevant','false_entity_match')
      )
  ),
  dim_agg AS (
    SELECT dimension,
  ...
  SELECT COALESCE(array_agg(e.id), '{}'::uuid[]) INTO v_input_ids
  FROM supplier_risk_events e
  WHERE e.supplier_id = p_supplier_id
    AND e.status IN ('open','accepted','under_review','remediated')
    AND NOT EXISTS (
      SELECT 1 FROM supplier_risk_feedback f
      WHERE f.risk_event_id = e.id AND f.buyer_id = p_buyer_id
        AND f.feedback_type IN ('not_relevant','false_entity_match')
    );
