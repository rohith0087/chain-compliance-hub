...
export function useComplianceDecisionsFeature(buyerId?: string) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: catalog, error: catalogError }, { data: override, error: overrideError }] = await Promise.all([
        client.from('feature_flags').select('default_enabled').eq('key', 'compliance_decisions_v1').maybeSingle(),
        client.from('organization_feature_flags').select('enabled, expires_at')
          .eq('organization_id', buyerId)
          .eq('organization_type', 'buyer')
          .eq('feature_key', 'compliance_decisions_v1')
          .maybeSingle(),
      ]);
