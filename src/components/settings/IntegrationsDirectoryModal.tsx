...
const INTEGRATIONS = [
  {
    ...
    popular: '#1 popular',
    logo: (
      <div className="w-10 h-10 rounded-xl bg-white border shadow-sm flex items-center justify-center p-2">
  ...
  },
  {
    ...
    new: true,
    logo: (
      <div className="w-10 h-10 rounded-xl bg-white border shadow-sm flex items-center justify-center p-1.5">
...
export function IntegrationsDirectoryModal({ open, onOpenChange }: IntegrationsDirectoryModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[1000px] h-[85vh] p-0 gap-0 overflow-hidden bg-[#FAFAFA] flex flex-col border-none shadow-2xl rounded-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-white relative z-10 shrink-0">
          ...
          <Button variant="ghost" size="icon" onClick={() => onOpenChange(false)} className="rounded-full hover:bg-slate-100 w-8 h-8">
            <X className="w-4 h-4" />
          </Button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar */}
          <div className="w-60 shrink-0 border-r bg-white p-4 hidden md:block">
            <nav className="space-y-1">
              <button className="flex items-center gap-3 w-full px-3 py-2 text-sm font-semibold text-slate-900 bg-slate-100 rounded-lg transition-colors">
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input 
                    ...
                    className="pl-10 h-11 bg-white border-slate-200 shadow-sm rounded-xl focus-visible:ring-1 focus-visible:ring-slate-300"
                    value={search}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <Badge variant="secondary" className="bg-slate-200/50 hover:bg-slate-200 text-slate-700 px-3 py-1 text-sm font-medium border-none">
                    TraceR2C & Partners
                  </Badge>
                  <div className="flex items-center gap-2">
                    <select className="h-9 px-3 rounded-lg border border-slate-200 bg-white text-sm text-slate-600 outline-none hover:bg-slate-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredIntegrations.map((integration) => (
                  <div 
                    key={integration.id}
                    className="group bg-white border border-slate-200 rounded-2xl p-5 hover:border-slate-300 hover:shadow-md transition-all cursor-pointer flex flex-col h-full"
                  >
