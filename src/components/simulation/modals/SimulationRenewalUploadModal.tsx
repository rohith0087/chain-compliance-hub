...
export const SimulationRenewalUploadModal = () => {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      submitRenewal(selectedExpiringDoc.id, fileName || `${selectedExpiringDoc.document_type}_v2.pdf`);
      setFileName('');
  ...
  return (
    <Dialog open={showRenewalUploadModal} onOpenChange={() => closeRenewalModal()}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          {/* Simulated File Name */}
          <div>
            <label className="text-sm font-medium">File Name (simulated)</label>
            <Input
              placeholder={`${selectedExpiringDoc.document_type}_v2.pdf`}
              value={fileName}
