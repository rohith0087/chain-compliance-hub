-- Create function to sync document upload status with request status
...
CREATE OR REPLACE TRIGGER sync_document_status_trigger
  AFTER UPDATE OF status ON document_requests
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status)
  EXECUTE FUNCTION sync_document_upload_status();

-- Create enhanced search function for better document relevance
