...
const ChatPage = () => {
  ...
  // Load chat sessions after company info is available
  useEffect(() => {
    if (companyInfo && user) {
      loadChatSessions();
    }
  ...
  // Determine which sidebar layout to use based on company type
  const SidebarLayout = companyInfo?.type === 'buyer' ? BuyerSidebarLayout : SupplierSidebarLayout;
  ...
  const sidebarUser = {
    roles: [companyInfo.type] as ("buyer" | "supplier")[],
    name: user.email?.split('@')[0] || 'User',
    currentRole: companyInfo.type as "buyer" | "supplier"
  };
        ...
        <div className="border-b border-border bg-card p-4">
          <div className="flex items-center justify-between max-w-6xl mx-auto">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-foreground">Compliance Compass</h1>
            ...
            {messages.length === 0 ? (
              ...
              <div className="space-y-1">
                {messages.map((message) => (
                  <div key={message.id} className="flex items-start gap-4 p-6 hover:bg-accent/30 transition-colors rounded-lg">
                ...
                {isLoading && (
                  <div className="flex items-start gap-4 p-6 bg-accent/20 rounded-lg">
