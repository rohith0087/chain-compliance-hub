...
- **Slice 5 ✅ DONE** — scoped `get_supplier_risk_graph(supplier)` RPC (supplier → events → sources → ownership edges as {nodes,edges}) + dependency-free SVG network view (`SupplierRiskGraph`) below the risk panel. Verified on Deb El (5 nodes / 4 edges). Migration `20260715002149_supplier_risk_graph_rpc.sql`. Ownership edges render when `risk_entities`/edges are populated (needs a corporate-registry/entity-resolution source — future).
- **Slice 6 ✅ mostly DONE (deployed to prod)** — shared ingest helper (`_shared/supplierRiskIngest.ts`) + CPSC recalls + openFDA food enforcement (targeted per-supplier search) connectors; OFAC refactored onto the helper. All 3 scheduled weekly. Fixed a real bug (partial index → `ON CONFLICT` 42P10). Produced real events (Deb El Food Products → 2 Class I FDA egg recalls → score 14, correctly time-decayed). **Slice 6b ✅ DONE (deployed)** — `ingest-adverse-media`: Google News per supplier → `aiComplete` (gpt-4o-mini, jsonMode) classifies ONLY (never scores) → schema/enum/range validation → events written as `under_review` / `evidence_status='ai_inferred'`. Untrusted text is delimited in `<untrusted_content>` with an explicit never-follow instruction and never triggers writes directly. Verified: 8 articles → 8 classified → 3 valid events (plant closures/fire → `operational`). Bounded batch (`{"limit":N}`) since AI calls are sequential. **Not yet scheduled** — needs per-supplier scan-tracking for rotation, else a cron would rescan the same N suppliers forever.
- **Slice 7 ✅ DONE** — buyer-scoped feedback on events (`relevant` / `not_relevant` / `false_entity_match`) + **scoring engine v2 honours it**: labelled events drop out of THAT buyer's score only (events stay shared knowledge). Verified: Deb El 14 → 8 after labelling 1 of 2 events. Migration `20260715033555_supplier_risk_scoring_engine_v2_feedback.sql`. (Predictive weight suggestions with human approval: later.)
- **Super-admin ✅ DONE** — platform-wide "Supplier Risk" tab: `get_all_suppliers_risk_overview()` (admin-gated) + all-suppliers table regardless of buyer connection.
