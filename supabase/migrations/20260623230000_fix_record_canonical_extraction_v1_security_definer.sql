-- record_canonical_extraction_v1 was security invoker; called by
-- process-evidence-extraction-jobs-v1 via service_role admin client.
-- service_role has no USAGE on schema private (by design), so any trigger
-- or policy on the tables this function writes to that references private.*
-- fails with "permission denied for schema private".
-- Fix: security definer (matching every other thin RPC in this codebase).
